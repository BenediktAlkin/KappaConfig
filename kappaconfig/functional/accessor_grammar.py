    ...
    for dot_split in dot_splits:
        ...
        if bracket_splits[0] != "":
            ...
            if not dict_accessor[0].isalpha():
                ...
                raise dict_accessor_has_to_start_with_letter_error(accessor, dict_accessor)
            if not dict_accessor.isidentifier():
                from ..errors import dict_accessor_has_to_be_identifier_error
                raise dict_accessor_has_to_be_identifier_error(accessor, dict_accessor)
        ...
        for bracket_split in bracket_splits[1:]:
            ...
            if bracket_split[-1] != "]":
                ...
                raise missing_closing_bracket_error(accessor, f"[{bracket_split}")
            list_accessor = bracket_split[:-1]
            try:
                int_accessor = int(list_accessor)
            except ValueError:
                from ..errors import list_accessor_has_to_be_int_error
                raise list_accessor_has_to_be_int_error(accessor, f"[{list_accessor}]")
